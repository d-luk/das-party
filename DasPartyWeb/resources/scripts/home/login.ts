///<reference path="../site/custom-event.ts"/>
module SpotifyLogin {
    import CEvent = CustomEvents.CEvent;
    ...
    function login(callback: (token: string) => any, forced?: boolean) {
        ...
        window.addEventListener("message", event => {
            const hash = JSON.parse((event.data) as any);
    ...
    let loginEvent = new CEvent();
    export function onLogin(handler:()=>void):void {
        if (loggedInUser) handler();
        else loginEvent.addHandler(handler);
    }
    ...
    function tryLogin(forced?: boolean) {
        login(accessToken => {
            getUserData(accessToken).then((response: SpotifyAPI.ILoginResponse) => {
